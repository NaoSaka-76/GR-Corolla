...
| 世界各地のイベント情報 | プレスリリース記事は除外し、オーナーズミーティング/ミートアップ、GR Corolla自体の試乗会・体験会、著名モーターショーへの展示・出展など「人が集まる」イベントに限定。見出しに日付表記がある記事を上位表示 | Google News RSS |
...
- **イベント情報の正確な日程・開催地は本文まで解析できていない**: Google News RSSは
  ...
  見出しに日付らしき表記(◯月◯日、開催中 等)がある記事を優先的に上位表示しているが、
  厳密なフィルタではないため、参加を検討する場合は必ず元記事で日程・開催地を確認すること。
