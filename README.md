...
| モータースポーツ | TC America(米)・ARA(米ラリー)・スーパー耐久 水素エンジンGRカローラ(日)のトピックス/レース結果/ランキング関連 | Google News RSS + 公式サイト検索リンク |
| YouTube 人気動画 / 新着動画 | 「GR Corolla」検索結果を再生数順・投稿日順に整理(サムネイル付き) | YouTube検索結果ページのベストエフォート・スクレイピング |
...
各記事の左端の色帯とバッジで一目で判別できる。あくまで見出し文のみに基づく自動推定であり、
参考値として利用すること。
...
- **モータースポーツのレース結果/ランキングは非公式**: TC America・ARA・スーパー耐久の
  公式リザルト/ポイントランキング表は各シリーズでサイト構造が異なり安定した自動取得が
  難しいため、ニュース記事ベースの速報に留めている。正式な最新順位はダッシュボード内の
  「公式ランキングを検索」リンクから確認すること。
