...
| モータースポーツ | TC America(米)・ARA(米ラリー)・スーパー耐久 水素エンジンGRカローラ(日)のトピックス/レース結果/ランキング/年間レース日程。新着順。TC Americaは公式サイト実データのドライバーズランキングをグラフ表示、ARAは公式サイト実データの上位3名(表彰台)を表示し、いずれもGR Corollaで参戦するドライバー/チームを目立たせて表示。「レース日程」「ランキング」はボタンで開閉するデフォルト非表示のパネルに変更。TC America・ARAには追加で「車両規定」(参戦クラスの技術規定概要)と「参戦車両」(GR Corolla TC / GR Corolla Rally RC2の解説・写真・主要スペック)のボタンも用意(いずれもデフォルト非表示、手動調査の静的情報) | Google News RSS + TC America・スーパー耐久・ARA 各公式サイト(標準スクレイピング) + 公式サイト検索/スケジュールリンク + 車両規定・参戦車両は手動調査の静的データ |
